...
const projects = [
  { name: "Hermes Mission Control", type: "AI infrastructure", status: "In progress", description: "A command center for coordinating agents, queues, approvals, memory, handoffs, and live work.", tags: ["Agents", "Orchestration", "Dashboard"] },
  { name: "Baseball Workshop", type: "Sports technology", status: "Built", description: "A video-based coaching workspace for players, parents, and coaches to turn swings into useful feedback.", tags: ["Video", "Coaching", "Analysis"] },
  { name: "Marcy", type: "Small business AI", status: "Built", description: "An AI receptionist that captures leads, answers questions, sends texts, and keeps owners in the loop.", tags: ["Leads", "SMS", "Automation"] },
...
export default function Home() {
  return (
    <main>
      ...
      <section className="work wrap" id="work"><div className="section-head"><div><p className="eyebrow">Selected work</p><h2>A growing shelf of experiments.</h2></div><p className="section-note">Some are shipped. Some are still becoming. All of them start with a real problem.</p></div><div className="grid">{projects.map((project, index) => <article className={`card card-${(index % 4) + 1}`} key={project.name}><div className="card-top"><span className="number">0{index + 1}</span><span className={`status ${statusClass[project.status]}`}>{project.status}</span></div><p className="card-type">{project.type}</p><h3>{project.name}</h3><p className="card-description">{project.description}</p><div className="tags">{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div></article>)}</div></section>
